...
#   - the glob form needs the test runner's glob support, which Node 20 (the
#     Lambda runtime, and what CI pins) lacks — it fails with
...
count=$(find dist -name '*.test.js' | wc -l | tr -d ' ')
...
exec find dist -name '*.test.js' -exec node --test {} +
